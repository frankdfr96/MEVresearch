...
class PriorityArea:
    ...
    def __init__(self, lastBlockNumber="", n_blocks=300000):
        self.blocks = self.load_blocks(lastBlockNumber, n_blocks)
    ...
    #n_blocks gets rounded up to the nearest 10k multiple
    def load_blocks(self, lastBlockNumber, n_blocks):
            try: 
                #Load Blocks from file
                f = open("blocksFrom={}Load={}.txt".format(lastBlockNumber, n_blocks), "r")
                ...
                load_size = 10000
                n_loads = ceil(n_blocks/load_size)
                latest = lastBlockNumber
                blocks = []
                
                for i in range(n_loads):
                    if i == n_loads - 1: load_size = n_blocks - (n_loads-1)*load_size
                ...
                f = open("blocksFrom={}Load={}.txt".format(blocks[0]["block_number"], n_blocks), "w+")
                f.write(json.dumps(blocks))
            finally:
                f.close()
                numToBlock = {block["block_number"]: block for block in blocks}
                all_blocks = []            
                nonFlashbotsBlock = {"miner_reward":"0", "gas_used":"0", "gas_price":"0", "transactions":[], "isFlashbotsBlock":False}
                for blockNum in range(blocks[-1]["block_number"], blocks[0]["block_number"]+1):
                    if blockNum in numToBlock:
                        all_blocks.append(numToBlock[blockNum])
                        all_blocks[-1]["isFlashbotsBlock"] = True
                    else:
                        all_blocks.append({"block_number":blockNum, "miner_reward":"0","gas_used":"0", \
                                           "gas_price":"0", "transactions":[], "isFlashbotsBlock":False})
                return all_blocks
    ...
    def print_results(self, burnt, priority_gas, gas_fee=None, one_off_fee = None, text=True, plot=True, title="Results", verbose=False):
        ...
        if text:
            ...
            print("Average burn percentage: {:.1f}%".format(100*np.mean(
                 [b/r for b,r in zip(burnt, self.rewards) if r > 0])))
            print("Total burn percentage: {:.1f}%".format(100*np.sum(burnt)/np.sum(self.rewards)))
            print("Mean priority gas utilization: {:.1f}%".format(100*np.mean(
                [pg/g for pg,g in zip(priority_gas, self.original_gas) if g > 0])))
            ...
            if verbose:
                ...
                print("Average burn percentage without outliers: {:.1f}%".format(100*np.mean(np.mean(
                 [b/r for b,r in zip(burnt[non_outliers], self.rewards[non_outliers]) if r > 0]))))
